...
bash your_path_to/simulation/simulation_protocol.sh -d ./ -r your_path_to/GRCh38_no_alt.fa -t your_path_to/GRCh38.transposon.fa -N 100 -R 0 --sub-N 5 --germline-count 3000 --avg-somatic-count 500 --min-distance 9000 --depth 50 --species human --protocol ccs --truncProb 0.1 --nestProb 0.1 --mode 1
```
...
Prepare the filelist based on the results of `Step 1.1`, for example, `filelist_example/simulation_filelist_GRCh38`. The required information include:
...
```shell
sbatch simulation_with_blades_GRCh38.sh
```

### 1.3 Merge raw reads from each chromosome

Prepare the required filelist for merging, for example, `filelist_example/merge_filelist_GRCh38`. The required information include:
...
```shell
sbatch merge_with_blades_GRCh38.sh

# Main results under training_dataset_x of each datasets:
# 1. TGS.fasta
# 2. chr1/chr1.ins.summary
# 3. chr1/chr1.ins.sequence
```

In our research, total 150 datasets were generated for Human using different `depth(10-50X)` and `protocol(ccs, clr, ont)`:
...
Prepare the required filelist for mapping, for example, `filelist_example/minimap2_filelist_GRCh38`. The required information include:
...
```shell
sbtach minimap2_with_blades_GRCh38.sh
```

### 2.2 Dtecet all candidates on local machine

Prepare the required filelist for detection, for example, `filelist_example/label_filelist_GRCh38`. The required information include:
...
Prepare the required filelist for labeling, for example, `filelist_example/label_filelist_GRCh38`.

Then, label candidates for each datasets:

```shell
sbatch label_with_blades_GRCh38.sh
```
