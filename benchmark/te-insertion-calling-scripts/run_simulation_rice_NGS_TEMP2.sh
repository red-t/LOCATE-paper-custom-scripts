...
do
    for dep in 1 2 3 4 5 10 20 30 40 50
    do
        ...
        if [ ! -f ${OUT_PATH}/TEMP2/${SAMPLE}_${dep}X.insertion.bed ];then
            ...
            echo "conda activate py2 && TEMP2 insertion -l ${LEFT} -r ${RIGHT} -I ${BWA_INDEX}/genome -g ${GENOME_FA} -R ${TE_ANNO_FA} -t ${REPEATMASKER_FILE} -o ${OUT_PATH}/TEMP2/${SAMPLE} -p ${SAMPLE}_${dep}X -d -c ${CPU} && conda deactivate"
        ...
        #         -a \
        #         -h ${ANNO_PATH}/${GENOME}/benchmark/${GENOME}_template_melt.fa \
        #         -bamfile ${DATA_PATH}/for_melt/${SAMPLE}_ngs.${dep}X.sort.bam \
        #         -n ${ANNO_PATH}/${GENOME}/benchmark/rice.genes.bed \
        #         -t ${path1}/rice/meltTEZip.ex_polyA/mei_list.txt \
        ...
        # echo -e "\n-----------\n" >> ${OUT_PATH_dir}/time_simulation_ngs_rice.TEMP2.txt
        # echo -e $(date) >> ${OUT_PATH_dir}/time_simulation_ngs_rice.TEMP2.txt
        # echo -e "map time\t"${time_map} >> ${OUT_PATH_dir}/time_simulation_ngs_rice.TEMP2.txt
        # echo -e "sample\tdepth\tTEMP2\tmelt" >> ${OUT_PATH_dir}/time_simulation_ngs_rice.TEMP2.txt
        # echo -e "${SAMPLE}\t${dep}\t${time_temp2}\t${time_melt}" >> ${OUT_PATH_dir}/time_simulation_ngs_rice.TEMP2.txt
